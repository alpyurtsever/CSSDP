function [ y_bar, out ] = AdaNGDSDP( C, b, Aop, Atop, alpha, r, k, varargin )
%ADANGDSDP Implements AdaNGD [L2017], for solving the model dual SDP 
%problem template in [DYTCU2019]. 
%
%ADANGDSDP requires at least 6 inputs: 
...
% - scalar k between 0 and 2 for the algorithm parameter (see Algorithm 2
%   in [L2017]). Note that "k=0" is equivalent to AdaGrad [DHS2011].
%   "k = 1.1" is recommended.
%
%ADANGDSDP also has optional property inputs. These properties should  
%be input as a tuple {'property-tag',value} after 6th input. For example 
%AdaNGDSDP(C, b, Aop, Atop, alpha, r, k, 'maxit', 100, 'eigstol', 1e-3)
...
%   ALPHA, ADAGRAD_D, and FLAG_DOUBLING.
%   See (input 'alpha' and properties 'D','doubling').
%
...
out.params.ADANGD_D = D;
out.params.ADANGD_k = k;
...
for t = 0:maxit
    ...
    if n > 500
        ...
        eigsArg = C - diag(y_t);
        [eigvecs,eigvals] = eig(eigsArg,'vector');
        [sigma_t,indmin] = min(eigvals);
        u_t = eigvecs(:,indmin);
    end
...
function [V,S,D] = recoverPrimal(eigsOp, n, r, optsEigs, Aop, b)
