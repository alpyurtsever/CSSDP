function [ y_bar, out ] = AdaGradSDP( C, b, Aop, Atop, alpha, r, varargin )
%ADAGRADSDP Implements AdaGrad [DHS2011] as described in Algorithm 1 in 
%[L2017], for solving the model dual SDP problem template in [DYTCU2019]. 
%
%ADAGRADSDP requires at least 5 inputs: 
...
%
%ADAGRADSDP also has optional property inputs. These properties should  
%be input as a tuple {'property-tag',value} after 5th input. For example 
%AdaGradSDP(C, b, Aop, Atop, alpha, r, 'maxit', 100, 'eigstol', 1e-3)
...
for t = 0:maxit
    ...
    dualVal = b'*y_t + alpha*min(sigma_t,0);
    ng = norm(g);
    ...
    % % Normally, the theory reveals the convergence of ergodic average
    weight = 1/(t+1);
...
% Nested functions
    ...
    function printError()
        ...
        for pIt = 1:length(errNames)
            if ~isnan(out.y_bar.(errNames{pIt})(ptr))
                fprintf(' % 5.3e |', out.y_bar.(errNames{pIt})(ptr) );
            else
                fprintf(' % 10s |', 'NaN' );
            end
        end
